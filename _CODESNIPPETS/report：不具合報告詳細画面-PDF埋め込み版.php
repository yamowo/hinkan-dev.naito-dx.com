...
add_filter('the_content', function ($content) {
  ...
  // --- EmbedPress: acf_attachment_file が PDF のときだけ埋め込みを追記 ---
  $acf_file = get_field('acf_attachment_file'); // ACF 返り値: 配列 or URL 文字列
  $pdf_url  = '';
  if (is_array($acf_file) && !empty($acf_file['url'])) {
      $pdf_url = $acf_file['url'];
  } elseif (is_string($acf_file) && filter_var($acf_file, FILTER_VALIDATE_URL)) {
      $pdf_url = $acf_file;
  }
  ...
  if ($pdf_url) {
      ...
      if ($ext === 'pdf') {
          $admin_ajax = admin_url('admin-ajax.php'); // 環境に依存しない
          $query_args = array(
              'action'         => 'get_viewer',
              'scrolling'      => '-1',
              'selection_tool' => '0',
          ...
          $hash_key = '#key=';

          $viewer_src = esc_url( add_query_arg($query_args, $admin_ajax) . '&file=' . rawurlencode($pdf_url) ) . $hash_key;

          $out .= '
  <!-- EmbedPress PDF埋め込みテンプレート START-->
  <div class="wp-block-embedpress-embedpress-pdf">
    <div class="embedpress-document-embed ep-doc-embedpress-pdf-1760054991143 ep-percentage-width"
        style="width:600%;max-width:100%"
        id="ep-doc-undefined"
        data-source-id="source-undefined"
        data-embed-type="PDF"
        data-embedpress-content="source-undefined">
      <div class="ep-embed-content-wraper">
        <div class="position-right-wraper gutenberg-pdf-wraper">
          <div class="main-content-wraper">
            <noscript>
              <iframe title="" style="height:600px;width:100%"
                      class="embedpress-embed-document-pdf embedpress-pdf-1760054991143"
                      data-emid="embedpress-pdf-1760054991143"
                      src="' . $viewer_src . '"></iframe>
            </noscript>
          </div>
        </div>
      </div>
